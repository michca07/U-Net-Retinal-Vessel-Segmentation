...
To enhance the robustness and generalization of the U-Net model, the training image set is enlarged through various augmentation processes. The following transformations are applied:

- **Rotate:**
  Training images are randomly rotated to simulate different orientations, aiding the model in learning invariant features.

- **Vertical Flip:**
  Randomly flipping images vertically helps the model generalize to variations in vessel orientation.

- **Horizontal Flip:**
  Random horizontal flips contribute to the model's ability to recognize vessels from different viewpoints.

- **Zoom In:**
  Random zooming into training images provides variations in scale, enabling the model to detect vessels at different magnifications.

- **Shear:**
  Introducing random shearing to training images helps the model adapt to deformations in vessel shapes.

- **Blur:**
  Introducing random blurring to training images helps the model adapt to the likely blur in images.

By applying these augmentation processes, the training dataset is diversified, preventing overfitting and improving the U-Net model's ability to segment retinal vessels across various conditions.
